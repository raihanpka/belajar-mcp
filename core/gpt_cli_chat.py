...
from core.gpt_chat import Chat
from core.gpt import GPT
from mcp_client import MCPClient


class CliChatGPT(Chat):
    def __init__(
        self,
        ...
        super().__init__(clients=clients, gpt_service=gpt_service)
    ...
    async def _process_command(self, query: str) -> bool:
        ...
        self.messages += convert_prompt_messages_to_openai_params(messages)
        return True
...
def convert_prompt_message_to_openai_param(
    ...
    if isinstance(content, dict) or hasattr(content, "__dict__"):
        ...
        if content_type == "text":
            ...
            return ChatCompletionMessage.create(role=role, content=content_text)
    ...
    if isinstance(content, list):
        ...
        if text_blocks:
            return ChatCompletionMessage.create(role=role, content=text_blocks)

    return ChatCompletionMessage.create(role=role, content="")


def convert_prompt_messages_to_openai_params(
    prompt_messages: List[PromptMessage],
) -> List[ChatCompletionMessage]:
    return [
        convert_prompt_message_to_openai_param(msg)
